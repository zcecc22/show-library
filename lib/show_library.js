...
ShowLibrary.prototype.getShowsID = function(cb) {
  this.pool.query('SELECT id, tvdb_id FROM shows', cb);
};

ShowLibrary.prototype.getEpisodesID = function(cb) {
  this.pool.query('SELECT id, show_id, season, episode FROM episodes', cb);
};
...
ShowLibrary.prototype.updateAllShows = function(done) {
  const self = this;
  self.getShowsID(function(err, rows) {
    if (err) {
      done(err);
      return;
    }
